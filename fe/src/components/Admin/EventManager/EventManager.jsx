...
// }
//}
// src/components/Admin/EventManager/EventManager.jsx
}
import React, { useEffect, useRef, useState } from "react";
import axios from "axios";
...
const normalizeEvent = (ev = {}) => ({
  ...
  title: ev.title ?? ev.name ?? "",
  type: ev.type ?? ev.event_type ?? "day", // day | month
  start: ev.start ?? ev.startAt ?? null,
  end: ev.end ?? ev.endAt ?? null,
  coins: Number(ev.coins ?? 0),
  participants: ev.participants ?? [],
  gameType: ev.gameType ?? ev.game_type ?? GAME_TYPES[0],
  ...ev,
...
export default function EventManager() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        setEvents(safeArr(raw).map(normalizeEvent));
      } catch (err) {
        console.warn("Fetch events failed", err);
        setMsg("Không thể tải danh sách sự kiện.");
        setEvents([]);
      } finally {
        setLoading(false);
      }
  ...
  const filtered = safeArr(events).filter((ev) => {
    if (search) {
      ...
      if (
        !(
          ...
          String(ev.gameType).toLowerCase().includes(q) ||
          String(ev.type).toLowerCase().includes(q) ||
          String(ev.coins).includes(q)
        )
      )
        return false;
    }
    ...
    if (filterFrom) {
      ...
      const evMs = ev.start
        ? new Date(ev.start).setHours(0, 0, 0, 0)
        : null;
      if (!evMs || evMs < fromMs) return false;
    }

    if (filterTo) {
      const toMs = new Date(filterTo + "T23:59:59").getTime();
      const evMs = ev.start ? new Date(ev.start).getTime() : null;
      if (!evMs || evMs > toMs) return false;
    }
  ...
  const viewDetails = async (id) => {
    ...
    try {
      const res = await axios.get(`/api/events/${id}`);
      setCurrent(normalizeEvent(res.data?.data ?? res.data));
    } catch {
      const local = events.find((e) => String(e.id) === String(id));
      setCurrent(local ? { ...local } : null);
      setMsg("Không lấy được chi tiết từ API, dùng dữ liệu cục bộ.");
    } finally {
      setLoadingDetail(false);
    }
  };
  ...
  const openEdit = async (id) => {
    ...
    try {
      const res = await axios.get(`/api/events/${id}`);
      const n = normalizeEvent(res.data?.data ?? res.data);
      setCurrent(n);
      setCurrentEndTime(
        n.end ? new Date(n.end).toISOString().slice(11, 16) : ""
      );
    } catch {
      const local = events.find((e) => String(e.id) === String(id));
      if (local) {
        setCurrent({ ...local });
        setCurrentEndTime(
          local.end ? new Date(local.end).toISOString().slice(11, 16) : ""
        );
      }
    } finally {
      setLoadingDetail(false);
  ...
  const save = async () => {
    ...
    try {
      if (mode === "create") {
        const res = await axios.post("/api/events", payload);
        const created = normalizeEvent(res.data?.data ?? payload);
        setEvents((p) => [created, ...p]);
      } else {
        await axios.put(`/api/events/${current.id}`, payload);
        setEvents((p) =>
          p.map((e) =>
            String(e.id) === String(current.id)
              ? normalizeEvent({ ...e, ...payload })
              : e
          )
        );
      }
      ...
      console.warn("Save failed, fallback local", err);
    } finally {
      setSaving(false);
    }
  };
