...
export default function AccountManager() {
  ...
  useEffect(() => {
    const fetchLists = async () => {
      ...
      try {
        ...
        const blkData =
          blkRes.data?.data ?? blkRes.data?.result ?? blkRes.data ?? [];
  ...
  const viewDetails = async (id) => {
    ...
    try {
      ...
      console.warn("Detail API failed, fallback local", err);
  ...
  const openEdit = async (id) => {
    ...
    try {
      const res = await axios.get(`/api/accounts/${id}`);
      setDetail({
        ...normalize(res.data?.data ?? res.data),
        password: "",
      });
    } catch (err) {
      console.warn("Edit API failed, fallback local", err);
      const local =
        safeArr(accounts).find((a) => Number(a.id) === Number(id)) ?? {};
      setDetail({ ...normalize(local), password: "" });
    } finally {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      console.warn("Save failed, update local", err);
      setAccounts((prev) =>
        prev.map((a) =>
          Number(a.id) === Number(detail.id)
            ? { ...a, ...detail }
  ...
  const deleteAccount = async (id) => {
    ...
    try {
      ...
      console.warn("Delete API failed, remove local", err);
    } finally {
      setAccounts((prev) => prev.filter((a) => Number(a.id) !== Number(id)));
      setReports((prev) => prev.filter((r) => r.accountId !== id));
      setBlocked((prev) => prev.filter((b) => Number(b.id) !== Number(id)));
