...
//   );
// }
// src/components/Admin/PostManager/PostManager.jsx
}
import React, { useEffect, useRef, useState } from "react";
import axios from "axios";
...
import BlockedPostsTable from "./BlockedPostsTable";
...
export default function PostsManager() {
  ...
  const fetchLists = async () => {
    ...
    try {
      ...
      setBlocked(bRes.data?.data ?? bRes.data ?? []);
    } catch (err) {
      console.warn(err);
      setMsg("Không thể tải dữ liệu từ API");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredBlocked = safeArr(blocked).filter((b) => {
    ...
    return (
      ...
      b.blockReason?.toLowerCase().includes(q)
    );
  });
  ...
  const save = async () => {
    if (!current?.title) return alert("Tiêu đề không được trống");

    setSaving(true);
    try {
      if (modalMode === "create") {
        const res = await axios.post("/api/posts", current);
        const created = res.data?.data ?? { ...current, id: Date.now() };
        setPosts((p) => [created, ...p]);
      } else {
        await axios.put(`/api/posts/${current.id}`, current);
        setPosts((p) =>
          p.map((x) => (x.id === current.id ? current : x))
        );
      }
      setModalOpen(false);
      setCurrent(null);
  ...
  const remove = async (id) => {
    if (!window.confirm("Xóa bài đăng?")) return;
    try {
      await axios.delete(`/api/posts/${id}`);
    } finally {
  ...
  const blockPost = async (id, reason) => {
    await axios.post(`/api/posts/${id}/block`, { reason });
    const post = posts.find((p) => p.id === id);
    setPosts((p) => p.filter((x) => x.id !== id));
    setBlocked((b) => [
      { ...post, blockReason: reason, blockedAt: new Date().toISOString() },
      ...b,
    ]);
  };
    ...
    <div>
      ...
      <div className="mb-3">
        <input
          ...
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="Tìm theo ID / tiêu đề / lý do"
