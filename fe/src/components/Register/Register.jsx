...
export default function Register({ onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    if (Object.values(conditions).includes(false))
    ...
    const data = {
      Phone: phone,
      Name: name,
      Password: bcrypt.hashSync(pswd, 5),
    };
  ...
  return (
    <div className="space-y-3">
      ...
      {errors.agree && (
        <p className="text-red-500 text-sm text-center">{errors.agree}</p>
      )}

      <button
        onClick={handleSubmit}
        className="w-full bg-pink-500 text-white rounded-lg py-2"
      >
        Đăng ký
      </button>
    </div>
